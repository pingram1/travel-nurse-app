...
const toneClasses: Record<BadgeTone, { bg: string; text: string }> = {
  success: { bg: 'bg-clinical-100', text: 'text-clinical-800' },
...
export function Badge({ label, tone = 'neutral', className }: BadgeProps) {
  ...
  return (
    <View className={`self-start rounded-full px-2.5 py-1 ${colors.bg} ${className ?? ''}`}>
      <Text className={`text-xs font-bold ${colors.text}`}>{label}</Text>
    </View>
  );
}
